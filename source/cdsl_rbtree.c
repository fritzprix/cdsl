...
#define PAINT_RED(node)                                      \
	do                                                       \
	{                                                        \
		node = (rbtreeNode_t *)((__cdsl_uaddr_t)node | RED); \
	} while (0)

#define PAINT_BLACK(node)                                     \
	do                                                        \
	{                                                         \
		node = (rbtreeNode_t *)((__cdsl_uaddr_t)node & ~RED); \
	} while (0)

#define PAINT_COLOR(node, color)                               \
	do                                                         \
	{                                                          \
		PAINT_BLACK(node);                                     \
...
static rbtreeNode_t *update_color(rbtreeNode_t *node_c)
{
	...
	if ((GET_COLOR(GET_PTR(node_c)->left) == RED) && (GET_COLOR(GET_PTR(node_c)->right) == RED))
	{
		return (rbtreeNode_t *)((__cdsl_uaddr_t)node_c & ~RED);
	}
	return node_c;
...
static rbtreeNode_t *resolve_black_black(rbtreeNode_t *parent_c, uint8_t dir_ctx, uint8_t *ctx)
{
	...
	if ((dir_ctx & 1) == CTX_LEFT)
	{
		...
		if ((GET_COLOR(GET_PTR(*sibling)->left) == BLACK) && (GET_COLOR(GET_PTR(*sibling)->right) == BLACK))
		{
